...
export const DETAILED_PROJECTS = {
  [PROJECTS_NAME.CENTAMESS]: {
    ...
    contributions: {
      en: [
        "Built real-time messaging features across Telegram, Facebook, Instagram, and LINE using WebSocket for low-latency communication, within a microservices-based NestJS architecture",
        ...
        "Standardized a reusable component system (20+ components) by customizing shadcn/ui with ReactJS and Tailwind CSS, applying consistent frontend design patterns to ensure scalable UI across modules",
        "Implemented bi-directional message routing with NestJS — receiving inbound messages from Facebook, Telegram, Instagram, and LINE via webhooks, persisting to the database, and broadcasting to the frontend in real-time via WebSocket; outbound replies from agents are routed back to the originating platform.",
        ...
        "Integrated carrier APIs (Viettel POST, GHN) for order fulfillment support.",
        "Built a multi-tenant embeddable live chat widget as a self-contained JavaScript bundle — each registered merchant receives a unique ID to inject the widget into their website, routing visitor messages directly into the Centamess inbox.",
        "Participated in sprint planning and code reviews within an Agile/Scrum team using Jira.",
      ],
      vi: [
        "Xây dựng tính năng nhắn tin thời gian thực trên Telegram, Facebook, Instagram và LINE sử dụng WebSocket để giảm độ trễ, trong kiến trúc NestJS theo mô hình microservices.",
        "Tích hợp OpenAI API với knowledge base tùy chỉnh để xây dựng chatbot chuyên biệt, tự động hóa phản hồi các câu hỏi thường gặp của khách hàng.",
        "Chuẩn hóa hệ thống component tái sử dụng (20+ component) bằng cách tùy chỉnh shadcn/ui với ReactJS và Tailwind CSS, áp dụng frontend design pattern nhất quán để đảm bảo UI có thể mở rộng xuyên suốt các module.",
        "Triển khai cơ chế định tuyến tin nhắn hai chiều với NestJS — nhận tin nhắn đến từ Facebook, Telegram, Instagram và LINE qua webhook, lưu vào cơ sở dữ liệu và phát trực tiếp lên frontend qua WebSocket; tin nhắn phản hồi từ agent được chuyển ngược về nền tảng gốc.",
        "Xây dựng service NestJS thu thập bình luận Facebook Livestream theo thời gian thực và tự động phân loại tin nhắn có ý định mua hàng để xử lý đơn.",
        "Phát triển pipeline di chuyển dữ liệu từ Sapo, nhập kho hàng, hồ sơ khách hàng, sản phẩm và đơn hàng vào cơ sở dữ liệu của nền tảng.",
        "Tích hợp API các đơn vị vận chuyển (Viettel POST, GHN) để hỗ trợ xử lý giao hàng.",
        "Xây dựng live chat widget nhúng đa tenant dưới dạng JavaScript bundle độc lập — mỗi merchant được cấp một ID riêng để nhúng widget vào website, định tuyến tin nhắn từ khách truy cập trực tiếp vào hộp thư Centamess.",
        "Tham gia lập kế hoạch sprint và review code trong nhóm Agile/Scrum sử dụng Jira.",
      ],
    },
    tags: [
      "React",
